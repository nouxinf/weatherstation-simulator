...
export function createFileBrowser(els, host) {
  ...
  function nodeToHtml(node, prefix, user, activePath) {
    ...
    for (const [name, child] of entries) {
      ...
      if (child && child.__dir) {
        ...
        html +=
          `<li><details class="tree-dir" data-path="${esc(full)}"${open ? " open" : ""}>` +
        ...
        html +=
          `<li><div class="tree-row${active}" tabindex="0" data-path="${esc(full)}" title="${esc(full)}">` +
        ...
        html +=
          `<li><div class="tree-row" tabindex="0" data-path="${esc(full)}" title="${esc(full)}">` +
          `<span class="row-name">${esc(name)}</span>` +
  ...
  function refresh(opts) {
    ...
    userList.querySelectorAll("details.tree-dir[data-path]").forEach((d) => {
      if (d.open) collapsedDirs.delete(d.dataset.path);
      else collapsedDirs.add(d.dataset.path);
    });
    ...
    const refocusPath =
      userList.contains(focused) && focused.classList.contains("tree-row")
    ...
    if (refocusPath) {
      userList
        .querySelector(
          `.tree-row[data-path="${refocusPath.replace(/["\\]/g, "\\$&")}"]`,
        )
        ?.focus();
    }
  ...
  function syncRows() {
    ...
    for (const el of [
      ...userList.querySelectorAll(".tree-row"),
      ...sysTree.querySelectorAll(".tree-row"),
    ]) {
  ...
  userList.addEventListener("click", (e) => {
    ...
    if (del) {
      e.stopPropagation();
      deleteUserPath(del.closest(".tree-row").dataset.path);
      return;
    }
  ...
  userList.addEventListener("contextmenu", (e) => {
    const row = e.target.closest(".tree-row");
    if (row) {
      showCtxMenu(e, row.dataset.path, false);
      return;
    }
  ...
  userList.addEventListener("keydown", (e) => {
    ...
    if (e.key === "Enter" || e.key === " ") {
      ...
      deleteUserPath(path);
      if (nextPath && !host.userFS.get(path)) {
        // deletion happened → move focus on
        userList
          .querySelector(
            `.tree-row[data-path="${nextPath.replace(/["\\]/g, "\\$&")}"]`,
          )
          ?.focus();
      }
    }
  ...
  sysTree.addEventListener("dblclick", (e) => {
    const row = e.target.closest(".tree-row");
    if (row)
      host.openFile(row.dataset.path, { transient: false, system: true });
  });
  sysTree.addEventListener("keydown", (e) => {
    if (e.key !== "Enter" && e.key !== " ") return;
    if (!e.target.classList.contains("tree-row")) return;
    e.preventDefault();
    host.openFile(e.target.dataset.path, {
      transient: e.key === " ",
      system: true,
    });
  });
  ...
  function showCtxMenu(e, path, isDir) {
    ...
    menu.querySelector('[data-action="open"]').style.display = isDir
      ? "none"
      : "";
  ...
  const dirOf = (t) =>
    t.isDir ? t.path : t.path.slice(0, t.path.lastIndexOf("/")); // dir to create into
  const onTarget = (fn) => () => {
    if (!ctxTarget) return;
    const t = ctxTarget;
    hideCtxMenu();
  ...
  function createUserFile(dirPrefix = "/") {
    ...
    if (name.includes("/")) {
      alert(
        "File name cannot contain /. Use the folder button to create directories.",
      );
      return;
    }
    const path = normalisePath(dirPrefix + name);
    if (host.userFS.get(path) && !confirm(path + " already exists. Overwrite?"))
      return;
  ...
  function autoSetTimeZone(dirPrefix = "/") {
    const name = "secrets.py";
    const path = normalisePath(dirPrefix + name);
    const offsetHours = Math.round(-new Date().getTimezoneOffset() / 60);
    host.userFS.set(path, {
      text: `# System default badge secrets.
#
# The secrets module imports /secrets first and falls back to this file, so
# create your own /secrets.py in the Files panel to override these.
WIFI_SSID = ""
WIFI_PASSWORD = ""
REGION = "eu"  # Options are us, cuba, eu, moldova, lebanon, egypt, chile, australia, nz
TIMEZONE = ${offsetHours}  # Offset from GMT as number of hours, i.e. 0, 1, -7 etc.
`,
      binary: false,
    });
  }
  ...
  return { refresh, syncRows, autoSetTimeZone };
}
