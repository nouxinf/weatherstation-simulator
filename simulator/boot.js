...
const SITE_STARTUP = "/system/apps/weatherstation/__init__.py";
...
export function bootSimulator() {
  ...
  _bootCtx = (async () => {
    ...
    const runIcons = document.querySelectorAll(
    ...
    const { applyCanvasToScreen, pauseScreen, rotateView } = initBadge3D(
      simulator,
      appendOut,
      badge3dWrap,
    );
    ...
    if (outputTitle && "ResizeObserver" in window) {
      const syncBadgeSpill = () =>
        badge3dWrap.style.setProperty(
          "--badge-spill",
          outputTitle.offsetHeight + "px",
        );
    ...
    const setRunning = (running) => {
      ...
      document.querySelectorAll('[data-action="stop"]').forEach((b) => {
        b.disabled = !running;
      });
      syncRunIcon();
    };
    ...
    const run = async () => {
      ...
      if (req)
        await runProgram(req.code, { tabKey: req.tabKey, status: req.status });
      else await runOS();
    };
    ...
    const hFile = location.hash
      ? decodeURIComponent(location.hash.slice(1))
      : "";
    const override = (SITE_STARTUP || qFile || hFile || "").trim();
      ...
      if (exMatch) {
        ...
        if (entry && !entry.binary && !entry.isDir) {
          ...
          defaultCode = await fetch(BOOT_BASE + "filesystem" + path)
            ...
            .catch(() => null);
          if (defaultCode != null)
            startupFile = { path, tabKey: "sys:" + path, system: true };
        }
      }
      if (defaultCode == null)
        warn = `⚠ Could not load "${override}" — running the default instead.`;
    }
    if (defaultCode == null) {
      startupFile = null;
      defaultCode =
        (await fetch(BOOT_BASE + "filesystem/system/main.py")
          .then((r) => (r.ok ? r.text() : null))
          .catch(() => null)) ??
    ...
    runProgram(defaultCode, {
      tabKey: startupFile ? startupFile.tabKey : null,
    });
